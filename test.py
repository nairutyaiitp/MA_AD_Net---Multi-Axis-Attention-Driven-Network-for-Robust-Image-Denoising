...
parser.add_argument('--test-dataset', type=str, default='./datasets/SIDD/test', help='dataset director for test')
parser.add_argument('--dataset-path', type=str, default='./datasets/SIDD/test', help='dataset director for test')
...
def test(model, test_loader, dataset=None):
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            image_ssim = calculate_ssim(pred, target, 0, test_y_channel=False)
            image_psnr = calculate_psnr(pred, target, 0, test_y_channel=False)
