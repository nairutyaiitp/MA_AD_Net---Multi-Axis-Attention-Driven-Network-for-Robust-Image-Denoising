...
# Explicitly Set GPU IDs (Change "5,6" to your available GPU IDs)
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = "5,6"  # Example: Using GPU 5 and 6
...
parser.add_argument('--train-dataset', type=str, default='./datasets/SIDD/train', help='dataset folder for train')
parser.add_argument('--val-dataset', type=str, default='./datasets/SIDD/test', help='dataset folder for val')
parser.add_argument('--save-epoch-freq', type=int, default=2 , help='how often to save model')
...
parser.add_argument('--epochs', type=int, default=40, help='total number of epoch')
...
def build_model(opt):
    ...
    model = PCFormer(channel=40).to(device)
    
    if len(device_ids) > 1:
        model = torch.nn.DataParallel(model, device_ids=device_ids)
    
    #checkpoint = torch.load('./checkpoints/pre_training.pth', map_location=device)
    ...
    loss = {'l1': L1Loss().to(device), 'l2': L2Loss().to(device), 'ssim': SSIMLoss().to(device),
            'carloss': CharbonnierLoss().to(device), 'edgeloss': EdgeLoss().to(device),
            'contrastloss': ContrastLoss().to(device), 'psnrloss': PSNRLoss().to(device)}
...
def train(epoch, model, train_loader, optimizer, train_loss, opt):
    ...
    for batch_iter, data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output_1, output_2, output_3 , output_4 = model(image)

        losses = train_loss['l1'](output_1, target) + \
                 train_loss['l1'](output_2, target) + \
                 train_loss['l1'](output_3, target) + \
                 train_loss['l1'](output_4, target)
...
def val(epoch, model, val_loader, val_loss, opt):
    ...
    for i, data in enumerate(val_loader):
        ...
        losses = val_loss['l1'](output_1, target) + \
                 val_loss['l1'](output_2, target) + \
                 val_loss['l1'](output_3, target) + \
                 val_loss['l1'](output_4, target)
...
def main(opt):
    ...
    val_loader = get_trainval_loader(opt)  # Add this for validation
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if opt.val and epoch % opt.val_freq == 0:
            epoch_val_loss = val(epoch, model, val_loader, loss, opt)  # Now val_loader exists
